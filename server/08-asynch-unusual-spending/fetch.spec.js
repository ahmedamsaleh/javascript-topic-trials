...
describe.only('fetch should', () => {
  it('interact with months and api', () => {
    ...
    when(apiWrapper(userId, priorMonth)).thenResolve(priorMonthsPayment);
    when(apiWrapper(userId, currentMonth)).thenResolve(currentMonthsPayment);
    ...
    return fetch(userId).should.eventually.deepEqual(payments);
